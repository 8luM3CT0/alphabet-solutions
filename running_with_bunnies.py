...
# space station -- and fast! Unfortunately, some of the bunnies have been 
# weakened by their long work shifts and can't run very fast. Their friends are 
# trying to help them, but this escape would go a lot faster if you also pitched in. 
# The defensive bulkhead doors have begun to close, and if you don't make it 
...
# arms, so picking them up is instantaneous, and arriving at the bulkhead at 
# the same time as it seals still allows for a successful, if dramatic, escape. 
...
# worker IDs (as indexes) in sorted order. The bunnies are represented as a 
# sorted list by worker ID, with the first bunny being 0. There are at most 5 
